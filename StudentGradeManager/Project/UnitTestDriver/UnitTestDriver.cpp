...
namespace UnitTestDriver
{
	...
	public:
		...
		TEST_METHOD(AddStudent_normal)
		{ //add one student
			StudentDB db;
			db_init(&db);
			Assert::AreEqual(1, db_addStudent(&db, 0001, "Sehejpreet"));
		}
		TEST_METHOD(Addstudent_morethanone)
		...
		TEST_METHOD(addstudent_checkifcorrect)
		{    //check if the entered data is stored correctly
			StudentDB db;
			db_init(&db);
       ...
       //edge cases
		...
		TEST_METHOD(Addstudent_gradezero)
		{   //grades of newly added student should be zero
			StudentDB db;
			db_init(&db);
			db_addStudent(&db, 0001, "Sehejpreet");
			Assert::AreEqual(0,db.students[0].gradeCount);
		}
		TEST_METHOD(Addstudent_avgzero)
		{   //average of new student should be zero
			StudentDB db;
			db_init(&db);
			db_addStudent(&db, 0001, "Sehejpreet");
		...
		//Invalid cases
		TEST_METHOD(Addstudent_duplicateid)
		{  // no two students can have same ids
			StudentDB db;
			db_init(&db);
			db_addStudent(&db, 0001, "Sehejpreet");
			Assert::AreEqual(INVALID_VALUE, db_addStudent(&db, 0001, "Prabhpreet"));
		}
		TEST_METHOD(Addstudent_negativeid)
		{ //no student can have negative id
			StudentDB db;
			db_init(&db);
			Assert::AreEqual(INVALID_VALUE, db_addStudent(&db,-1, "Sehejpreet"));
		}
		TEST_METHOD(addStudent_zeroid)
		{  //no student can have zero as their id
			StudentDB db;
			db_init(&db);
	...
	public:
		...
		TEST_METHOD(RemoveStudent_Normal)
		{ //remove student
			StudentDB db;
			db_init(&db);
			db_addStudent(&db, 0001, "Sehejpreet");
		...
		TEST_METHOD(RemoveStudent_CountDecrease)
		{
			// After removal the student count should decrease by 1
			StudentDB db;
			db_init(&db);
			db_addStudent(&db, 0001, "Sehejpreet");
		...
		TEST_METHOD(RemoveStudent_RemainingStudentShiftedCorrectly)
		{
			// After removing the first student the second should shift to index 0
			StudentDB db;
			db_init(&db);
			db_addStudent(&db, 0001, "Sehejpreet");
			db_addStudent(&db, 0002, "Prabhpreet");
		...
		TEST_METHOD(RemoveStudent_LastStudent_CountBecomesZero)
		{
			// Removing the only student should result in count = 0
			StudentDB db;
			db_init(&db);
			db_addStudent(&db, 0001, "Sehejpreet");
			db_removeStudent(&db, 0001);
		...
		TEST_METHOD(RemoveStudent_NotFound_ReturnsInvalid)
		{ // removing the student which is not in the list will return invalid
			StudentDB db;
			db_init(&db);
			Assert::AreEqual(INVALID_VALUE, db_removeStudent(&db, 9999));
		}

		TEST_METHOD(RemoveStudent_EmptyDatabase_ReturnsInvalid)
		{ // student cannot be removed from empty list
            StudentDB db;
			db_init(&db);
	...
	public:
		...
		TEST_METHOD(RecordGrade_Normal)
		{ //grades added 
            StudentDB db;
			db_init(&db);
			db_addStudent(&db, 0001, "Sehejpreet");
		...
		TEST_METHOD(RecordGrade_GradeCountIncrease)
		{  // when grades are added count should increase by 1
			StudentDB db;
			db_init(&db);
			db_addStudent(&db, 0001, "Sehejpreet");
		...
		TEST_METHOD(RecordGrade_Average)
		{  //calculate average
			StudentDB db;
			db_init(&db);
			db_addStudent(&db, 0001, "Sehejpreet");
		...
		TEST_METHOD(RecordGrade_MinGrade)
		{  // 0 is the minimum marks
			StudentDB db;
			db_init(&db);
		...
		TEST_METHOD(RecordGrade_MaxGrade)
		{ //100 is themaximum marks
			StudentDB db;
			db_init(&db);
		...
		TEST_METHOD(RecordGrade_StudentNotFound)
		{ // grades for a student who is not in the data cannot be added
			StudentDB db;
			db_init(&db);
			Assert::AreEqual(INVALID_VALUE, db_recordGrade(&db, 9999, 85.0f));
		}

		TEST_METHOD(RecordGrade_NegativeGrade)
		...
		TEST_METHOD(RecordGrade_GradeAbove100)
		{ //grades cannot be beyong 100
			StudentDB db;
			db_init(&db);
			db_addStudent(&db, 0001, "Sehejpreet");
	...
	public:
		...
		TEST_METHOD(CalcAverage_NormalCase)
		{
			float grades[] = { 80.0f, 90.0f, 70.0f };
			Assert::AreEqual(80.0f, db_Average(grades, 3), 0.001f);
		}

		TEST_METHOD(CalcAverage_SingleGrade)
		{
			// Average of one grade should be the grade itself
			float grades[] = { 95.0f };
			Assert::AreEqual(95.0f, db_Average(grades, 1), 0.001f);
		}

		TEST_METHOD(CalcAverage_AllSameGrade)
		{
			// Average of identical grades should equal that grade
		...
		TEST_METHOD(CalcAverage_AllZeros)
		{
			// Average of all zeros should be 0
			float grades[] = { 0.0f, 0.0f, 0.0f };
			Assert::AreEqual(0.0f, db_Average(grades, 3), 0.001f);
		}

		TEST_METHOD(CalcAverage_All100)
		{
			// Average of all 100s should be 100
		...
		TEST_METHOD(CalcAverage_ZeroCount)
		{
		...
		TEST_METHOD(CalcAverage_NegativeCount)
		{
		...
		TEST_METHOD(CalcAverage_NullGrades)
		{
			// Null pointer for grades array is invalid
